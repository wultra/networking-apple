...
- The project ships through SPM and CocoaPods, and release prep verifies README metadata too. On development branches, `WultraPowerAuthNetworking.podspec`, `Sources/WultraPowerauthNetworking/WPNConstants.swift`, and `WultraPowerAuthNetworking.xcodeproj/WultraPowerAuthNetworking_Info.plist` use `0.0.1-dev`; use `scripts/prepare-release.sh -v VERSION` for a release and `scripts/prepare-release.sh --prepare-dev` afterwards.
