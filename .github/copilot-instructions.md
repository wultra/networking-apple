...
- The project ships through SPM and CocoaPods, and release prep verifies README metadata too. On development branches, `WultraPowerAuthNetworking.podspec` and `Sources/WultraPowerauthNetworking/WPNConstants.swift` use `0.0.1-dev`, while `CFBundleShortVersionString` in `WultraPowerAuthNetworking.xcodeproj/WultraPowerAuthNetworking_Info.plist` remains the numeric `0.0.1`. Release preparation updates all three to the numeric release version; `scripts/prepare-release.sh --prepare-dev` restores only the podspec and source constant to `0.0.1-dev`.
